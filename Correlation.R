...
bike_s_aggr = apply(bicycle_start_sep,1,sum)
bike_e_aggr = apply(bicycle_end_sep,1,sum)

taxi_s_aggr = apply(taxi_start_sep,1,sum)
taxi_e_aggr = apply(taxi_end_sep,1,sum)
...
plot.cointegration <-function(){
  ...
  ts.tw = compute.cointegration(taxi_start_sep, t_mat)
  ts.tw.vec = urdf2vec(ts.tw)
  
  bs.tw = compute.cointegration(bicycle_start_sep, t_mat)
  bs.tw.vec = urdf2vec(bs.tw)
  
  ts.bs = compute.cointegration(taxi_start_sep, bicycle_start_sep)
